...
object ImportGpx {
	...
	fun loadGPXFileFromKml(kml: Source): Pair<GpxFile, Long> {
		val string = convertKmlToGpx(kml)
		if (string != null) {
	...
	private fun convertKmlToGpx(kml: Source): String? {
		return try {
			...
			println("Error during KML to GPX conversion: ${e.message}")
			e.printStackTrace()
			null
		}
	}
			...
			XmlPullParser.START_TAG -> {
				when (parser.getName()) {
					...
					"name" -> {
						...
						else if (folderNameStack.isNotEmpty()) folderNameStack[folderNameStack.lastIndex] = parser.nextText()
...
private fun processPlacemark(name: String?, desc: String?, coords: String) {
	...
	val placemark = if (coordinatePoints.size == 1) {
		// It's a Waypoint (<wpt>)
		Placemark(name, desc, folderNameStack.lastOrNull(), point = coordinatePoints.first())
	} else {
		// It's a Track (<trk>)
...
/**
 * Parses a KML coordinates string into a list of PointData objects.
 * Handles single points, space-delimited tracks, and comma-or-space-separated values.
 */
...
/**
 * Builds the final GPX XML string from the parsed placemarks.
 */
private fun buildGpx(): String {
	...
	serializer.startTag(null, "gpx").apply {
		...
		attribute(null, "creator", "Kotlin KML Converter")
	}

 ...
 *
 * What stays in `RouteResultPreparation` in OsmAnd-java is the orchestration - it holds the routing
 * context's tile machinery and the reader that a road's region is initialised from - along with the
 * area routing, the debug printing, and `convertFinalSegmentToResults`, which walks the java
 * planner's own segments.
 */

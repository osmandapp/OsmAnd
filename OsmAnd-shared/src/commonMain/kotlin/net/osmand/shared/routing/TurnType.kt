 ...
 *
 * The primary constructor is called from JNI by the C++ router in `core-legacy`
 * (`java_wrap.cpp`, descriptor `(IIFZ[IZZ)V`). Adding, removing or reordering its parameters, or
 * introducing a default value for any of them, changes that descriptor and breaks native routing on
 * Android at runtime with no compile time warning. `TurnTypeTest` pins the parameter order.
 *
...
class TurnType(
	...
	companion object {
		...
		fun lanesFromString(lanesString: String?): IntArray? {
			...
			// dropLastWhile keeps the trailing empty behaviour of Java's String.split
			val lanesArr = lanesString!!.split("|").dropLastWhile { it.isEmpty() }
			val lanes = IntArray(lanesArr.size)
			for (l in lanesArr.indices) {
				val turns = lanesArr[l].split(",").dropLastWhile { it.isEmpty() }

...
class MediaDetailsContentBuilder(private val app: OsmandApplication) {
	...
	fun build(item: MediaItem, metadata: GalleryMediaMetadata?, entry: MediaLibraryEntry?): List<DetailsItem> {
		...
		items += fileCardRow(R.string.shared_string_name, item.title, middleEllipsis = true)
		metadata?.format?.let { items += fileCardRow(R.string.shared_string_format, it) }
		(metadata?.creationTimeMs ?: metadata?.lastModifiedTimeMs)?.let {
			items += fileCardRow(R.string.shared_string_created, OsmAndFormatter.getFormattedDateTime(app, it))
		}
		metadata?.sizeBytes?.takeIf { it > 0 }?.let { items += fileCardRow(R.string.shared_string_size, AndroidUtils.formatSize(app, it)) }
		if (item.type == MediaType.PHOTO || item.type == MediaType.VIDEO) {
			MediaFormatting.resolution(metadata?.width, metadata?.height)?.let { items += fileCardRow(R.string.shared_string_resolution, it) }
		}
		...
		details?.author?.takeIf { it.isNotBlank() }?.let { items += fileCardRow(R.string.shared_string_author, it) }
		details?.date?.let { date ->
			val formatted = WikiAlgorithms.formatWikiDate(date).takeUnless { Algorithms.isEmpty(it) } ?: date
			if (formatted.isNotBlank()) items += fileCardRow(R.string.shared_string_added, formatted)
		}
		...
		if (!source.isNullOrEmpty() || sourceIcon != 0) {
			items += fileCardRow(R.string.shared_string_source, source.orEmpty(), iconId = sourceIcon, tintIcon = false)
		}
		details?.license?.takeIf { it.isNotBlank() }?.let { items += fileCardRow(R.string.shared_string_license, it) }
	...
	private fun fileCardRow(labelId: Int, value: String, iconId: Int = 0, tintIcon: Boolean = true, action: RowAction? = null,

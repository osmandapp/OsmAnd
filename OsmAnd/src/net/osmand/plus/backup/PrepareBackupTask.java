...
public class PrepareBackupTask {
	...
	public interface OnPrepareBackupListener {
		void onBackupPrepared(@Nullable BackupInfo backupInfo, @Nullable List<RemoteFile> userFiles,
							  @Nullable List<LocalFile> fileInfos, @Nullable String error);
	}
	...
	private void onTasksDone() {
		if (listener != null) {
			listener.onBackupPrepared(result, remoteFiles, fileInfos, error);
		}
		Context ctx = contextRef.get();
		if (ctx instanceof Activity && AndroidUtils.isActivityNotDestroyed((Activity) ctx) && progress != null) {
			progress.finishTask();

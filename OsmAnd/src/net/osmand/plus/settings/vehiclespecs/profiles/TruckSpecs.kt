...
class TruckSpecs : VehicleSpecs() {
    ...
    override fun collectSpecs() {
        ...
        add(
            type = SpecificationType.AXLE_LOAD,
            iconDayId = R.drawable.img_help_weight_limit_day,
            iconNightId = R.drawable.img_help_weight_limit_night,
        ...
        add(
            type = SpecificationType.WEIGHT_FULL_LOAD,
            iconDayId = R.drawable.img_help_weight_limit_day,
            iconNightId = R.drawable.img_help_weight_limit_night,
    ...
    override fun checkValue(
        ...
        if (type == SpecificationType.WEIGHT) {
            ...
            if (value < min) {
                val errorMessagePattern = ctx.getString(R.string.common_weight_limit_error)
                val minFormatted = units.formatValue(min)
                val unitsName = ctx.getString(units.getNameResId())
                val profileName = ctx.getString(ApplicationMode.CAR.nameKeyResource)

                error.append(String.format(errorMessagePattern, minFormatted, unitsName, profileName))
                return false
            }
        }
        return true

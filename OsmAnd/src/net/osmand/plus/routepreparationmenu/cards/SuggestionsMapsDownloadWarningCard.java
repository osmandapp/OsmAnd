...
public class SuggestionsMapsDownloadWarningCard extends WarningCard {
	boolean isDirectLinePoints;
	boolean isStartFinishIntermediates;
	private SelectionBottomSheet dialog;

	public SuggestionsMapsDownloadWarningCard(@NonNull MapActivity mapActivity) {
		super(mapActivity);
		isDirectLinePoints = !Algorithms.isEmpty(mapActivity.getMapRouteInfoMenu().getMissingMapsOnDirectLine());
		isStartFinishIntermediates = !Algorithms.isEmpty(mapActivity.getRoutingHelper().getRoute().getDownloadMaps());
		if (isStartFinishIntermediates) {

...
public class PanoramaxVectorLayer extends MapTileLayer implements PanoramaxLayer, IContextMenuProvider {
	...
	PanoramaxVectorLayer(@NonNull Context context) {
		super(context, false);
		plugin = PluginsHelper.getPlugin(PanoramaxPlugin.class);
	}
	...
	public void drawTileMap(Canvas canvas, RotatedTileBox tileBox, DrawSettings drawSettings) {
		TileSourceTemplate panoramaxTemplate = TileSourceManager.getPanoramaxVectorSource();
		...
		if (currentZoom < Math.max(panoramaxTemplate.getMinimumZoomSupported(), map.getMinimumZoomSupported())
				|| currentZoom > Math.min(panoramaxTemplate.getMaximumZoomSupported(), map.getMaximumZoomSupported())) {
			return;
		}
	...
	public PointDescription getObjectName(Object o) {
		if (o instanceof PanoramaxImage) {
			String description = getContext().getString(R.string.panoramax_image);
			return new PointDescription(PointDescription.POINT_TYPE_PANORAMAX_IMAGE, description);
		}
		return null;
	}
	...
	public LatLon getObjectLocation(Object o) {
		if (o instanceof PanoramaxImage image) {
	...
	private void collectImagesFromPoint(@NonNull MapSelectionResult result) {
		MapRendererView mapRenderer = getMapRenderer();
		if (mapRenderer != null && panoramaxTilesProvider != null) {
	...
	private void getImagesFromPointOpenGL(@NonNull MapSelectionResult result) {
		...
		QuadTree<PanoramaxImage> quadTree = panoramaxTilesProvider.getQuadTreeByPoint(center31);
		if (quadTree == null) {
			return;
		}

		List<PanoramaxImage> res = new ArrayList<>();
		...
		PanoramaxImage closestImage = null;
		for (PanoramaxImage image : res) {
			if (image == null) {
				continue;
			}
		...
		for (Entry<QuadPointDouble, Map<?, ?>> entry : points.entrySet()) {
			...
			if (Math.abs(x - ex) <= radius && Math.abs(y - ey) <= radius) {
				...
				if (image == null || minSqDist > sqDist) {
					...
					image = new PanoramaxImage(lat, lon);

...
public class PanoramaxAutoCompleteAdapter extends ArrayAdapter<String> {

	private static final String TAG = PanoramaxAutoCompleteAdapter.class.getSimpleName();
	...
	public PanoramaxAutoCompleteAdapter(@NonNull Context context, @LayoutRes int resource) {
				...
				if (constraint != null) {
					try {
						...
						if (!settings.isInternetConnectionAvailable()) {
							...
							GetPanoramaxUserAsyncTask task = new GetPanoramaxUserAsyncTask();
							Pair<String, String> user = OsmAndTaskManager.executeTask(task, constraint.toString()).get();
							if (user != null) {
								plugin.PANORAMAX_FILTER_USER_KEY.set(user.first);
								plugin.PANORAMAX_FILTER_USERNAME.set(user.second);

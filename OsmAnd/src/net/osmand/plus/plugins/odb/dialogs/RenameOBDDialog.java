...
public class RenameOBDDialog extends BaseOsmAndDialogFragment {
	...
	private void onSaveEditedText(@NonNull String newName) {
		...
		if (target instanceof OnSensorNameChangedCallback callback && property != null) {
			callback.onSensorNameChanged();
		}
	}

	public static void showInstance(@NonNull FragmentActivity activity,
	                                @Nullable Fragment target,
	                                @NonNull BTDeviceInfo device) {
		if (!(target instanceof OnSensorNameChangedCallback)) {
			throw new IllegalArgumentException("target fragment should implement OnSaveSensorNameCallback");
		}
		FragmentManager fragmentManager = activity.getSupportFragmentManager();
		if (!fragmentManager.isStateSaved()) {
			RenameOBDDialog fragment = new RenameOBDDialog();
			Bundle args = new Bundle();
			ExternalSensorsPlugin plugin = PluginsHelper.getPlugin(ExternalSensorsPlugin.class);
			if (plugin != null) {
				args.putString(DEVICE_ID_KEY, device.getAddress());
	...
	public interface OnSensorNameChangedCallback {
		void onSensorNameChanged();
	}
}

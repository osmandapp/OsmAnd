...
object NearbyPlacesHelper {
	...
	fun createSmallPointBitmap(layer: NearbyPlacesLayer): Bitmap {
		val borderWidth = layer.smallIconBorderSize
		...
		dstRect = RectF(borderWidth.toFloat(), borderWidth.toFloat(), (smallIconSize - borderWidth * 2).toFloat(), (smallIconSize - borderWidth * 2).toFloat())
		canvas.drawBitmap(circle, srcRect, dstRect, bitmapPaint)
	...
	fun createBigBitmap(layer: NearbyPlacesLayer, loadedBitmap: Bitmap): Bitmap {
		val borderWidth = layer.bigIconBorderSize
		val bitmapPaint = layer.bitmapPaint
		val circle = layer.circle
		...
		val cx = bg.width / 2
		val cy = bg.height / 2
		val radius = (min(cx.toDouble(), cy.toDouble()) - borderWidth * 2).toInt()
		canvas.save()

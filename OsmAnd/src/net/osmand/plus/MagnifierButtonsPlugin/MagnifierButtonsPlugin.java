		...
		displayZoomInButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(getDistanceCalculatorLayer()){
					if(distanceCalculatorLayer.getTextView().getLineCount() > 0) distanceCalculatorLayer.layoutText();
				}
	...
	private void showDialog(final MapActivity activity) {
		...
		bld.setSingleChoiceItems(values.toArray(new String[values.size()]), i,
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				dialog.dismiss();
				activity.getMapLayers().getContextMenuLayer().initLayer(activity.getMapView());
				if(getDistanceCalculatorLayer()) distanceCalculatorLayer.initLayer(view);
				updateZoomScaleTextbox(false);
			}
		});
		bld.show();

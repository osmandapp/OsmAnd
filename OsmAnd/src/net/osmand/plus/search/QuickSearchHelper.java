...
public class QuickSearchHelper implements ResourceListener {
	...
	public static class SearchHistoryAPI extends SearchBaseAPI {
		...
		public boolean search(SearchPhrase phrase, SearchResultMatcher resultMatcher) throws IOException {
			int p = 0;
			for (HistoryEntry entry : SearchHistoryHelper.getInstance(app).getHistoryEntries()) {
				SearchResult sr = new SearchResult(phrase);
				if (entry.getType() == HistoryEntry.POINT_TYPE) {
					PointHistoryEntry point = (PointHistoryEntry) entry;
					sr.localeName = point.getPointDescription().getName();
					sr.object = point;
					sr.objectType = ObjectType.RECENT_OBJ;
					sr.location = new LatLon(point.getLat(), point.getLon());
					sr.preferredZoom = 17;
				} else if (entry.getType() == HistoryEntry.CATEGORY_TYPE) {
					AbstractPoiType pt = MapPoiTypes.getDefault().getAnyPoiTypeByKey(entry.getName());
					if (pt != null) {

...
import net.osmand.plus.card.color.palette.gradient.DeleteGradientTask;
import net.osmand.plus.card.color.palette.gradient.DeleteGradientTask.DeleteGradientListener;
...
public class ColorPaletteHelper {
	...
	public void duplicateGradient(@NonNull PaletteGradientColor gradientColor, @NonNull DuplicateGradientListener duplicateGradientListener) {
		String colorPaletteFileName = ROUTE_PREFIX + gradientColor.getColorizationTypeName() + GRADIENT_ID_SPLITTER + gradientColor.getPaletteName() + EXT;
		File gradientToDuplicate = new File(getColorPaletteDir(), colorPaletteFileName);

		DuplicateGradientTask duplicateGradientTask = new DuplicateGradientTask(app, gradientToDuplicate, duplicateGradientListener);
	...
	public void deleteGradient(@NonNull PaletteGradientColor gradientColor, @NonNull DeleteGradientListener deleteGradientListener) {
		...
		DeleteGradientTask deleteGradientTask = new DeleteGradientTask(gradientToDuplicate, deleted -> {
			if (deleted) {
				cachedRouteColorPalette.remove(colorPaletteFileName);
			}
			deleteGradientListener.onGradientDeleted(deleted);
		});
		deleteGradientTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
	}
}

				...
				if (!useIntermediatePointsRTE) {
					for (Track tr : file.tracks) {
						if (!tr.generalTrack) {
							...
							} else if (tr.segments.size() > selectedSegment) {
								TrkSegment tkSeg = tr.segments.get(selectedSegment);
								for (WptPt pt : tkSeg.points) {
									points.add(createLocation(pt));
								}
							}
	...
	private static List<RouteSegmentResult> parseOsmAndGPXRoute(List<Location> points, GPXFile gpxFile, int selectedSegment) {
		for (Track tr : gpxFile.tracks) {
			if (selectedSegment == -1) {
				...
				TrkSegment ts = tr.segments.get(selectedSegment);
				for (WptPt pt : ts.points) {
					points.add(createLocation(pt));
				}
			}
		}
		List<TrkSegment> segments = gpxFile.getNonEmptyTrkSegments(true);
		if (selectedSegment>=0&&segments.size() > selectedSegment) {
			RouteImporter routeImporter = new RouteImporter(segments.get(selectedSegment));

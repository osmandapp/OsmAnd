...
public class OsmandMapTileMiniView implements MapTileDownloader.IMapDownloaderCallback {
    ...
    protected static final Log LOG = PlatformUtil.getLog(net.osmand.plus.views.OsmandMapTileView.class);
    ...
    private net.osmand.plus.views.OsmandMapTileView.OnLongClickListener onLongClickListener;

    private net.osmand.plus.views.OsmandMapTileView.OnClickListener onClickListener;

    private net.osmand.plus.views.OsmandMapTileView.OnTrackBallListener trackBallDelegate;

    private AccessibilityActionsProvider accessibilityActions;

    private List<OsmandMapMiniLayer> layers = new ArrayList<>();

    private BaseMapLayer mainLayer;

    private Map<OsmandMapMiniLayer, Float> zOrders = new HashMap<OsmandMapMiniLayer, Float>();

    private net.osmand.plus.views.OsmandMapTileView.OnDrawMapListener onDrawMapListener;
    ...
    public OsmandMapTileMiniView(OsmandApplication app, int w, int h) {
    ...
    public void init(int w, int h) {
        ...
        paintGrayFill.setStyle(Paint.Style.FILL);
        // when map rotate
...
//		paintImg.setDither(true);
        ...
        //baseHandler = new Handler(application.getResourceManager().getRenderingBufferImageThread().getLooper());
         ...
         WindowManager mgr = (WindowManager) application.getSystemService(Context.WINDOW_SERVICE);
    ...
    public boolean isLayerVisible(OsmandMapMiniLayer layer) {
        return layers.contains(layer);
    }

    public float getZorder(OsmandMapMiniLayer layer) {
    ...
    public synchronized void addLayer(OsmandMapMiniLayer layer, float zOrder) {
        int i = 0;
    ...
    public synchronized void removeLayer(OsmandMapMiniLayer layer) {
    ...
    public List<OsmandMapMiniLayer> getLayers() {
        return layers;
    }

    @SuppressWarnings("unchecked")
    public <T extends OsmandMapMiniLayer> T getLayerByClass(Class<T> cl) {
        for (OsmandMapMiniLayer lr : layers) {
            if (cl.isInstance(lr)) {
                return (T) lr;
            }
        }
    ...
    public boolean mapGestureAllowed(OsmandMapMiniLayer.MapGestureType type) {
//        for (OsmandMapMiniLayer layer : layers) {
...
//        }
    ...
    public void setOnDrawMapListener(net.osmand.plus.views.OsmandMapTileView.OnDrawMapListener onDrawMapListener) {
...
//			final float y2 = calc.getPixYFromLatLon(rb.getLatitude(), rb.getLongitude());
    ...
    private void refreshBaseMapInternal(RotatedTileBox tileBox, OsmandMapMiniLayer.DrawSettings drawSettings) {
        ...
        if (bufferBitmapTmp == null || tileBox.getPixHeight() != bufferBitmapTmp.getHeight()
            ...
            bufferBitmapTmp = Bitmap.createBitmap(tileBox.getPixWidth(), tileBox.getPixHeight(), Bitmap.Config.ARGB_8888);
        }
        ...
        for (int i = 0; i < layers.size(); i++) {
            try {
                OsmandMapMiniLayer layer = layers.get(i);
                canvas.save();
                // rotate if needed
    ...
    public void refreshMapInternal(OsmandMapMiniLayer.DrawSettings drawSettings) {
        if (view == null) {
            return;
        }
        final float ratioy;
    ...
    private void fillCanvas(Canvas canvas, OsmandMapMiniLayer.DrawSettings drawSettings) {
    ...
    public void drawOverMap(Canvas canvas, RotatedTileBox tileBox, OsmandMapMiniLayer.DrawSettings drawSettings) {
        if (mapRenderer == null) {
            fillCanvas(canvas, drawSettings);
        }
        final QuadPoint c = tileBox.getCenterPixelPoint();
    ...
    private void refreshBufferImage(final OsmandMapMiniLayer.DrawSettings drawSettings) {
        if (mapRenderer != null) {
            return;
        }

        OsmandMapMiniLayer.DrawSettings param = drawSettings;
        Boolean currentNightMode = nightMode;
        param = new OsmandMapMiniLayer.DrawSettings(currentNightMode, true);
...
//                                param = new OsmandMapMiniLayer.DrawSettings(drawSettings.isNightMode(), true);
//                            }
...
//        }
    ...
    public void refreshMap(final boolean updateVectorRendering) {
            ...
            OsmandMapMiniLayer.DrawSettings drawSettings = new OsmandMapMiniLayer.DrawSettings(nightMode, updateVectorRendering || forceUpdateVectorDrawing);
            //sendRefreshMapMsg(drawSettings, 20);
            refreshBufferImage(drawSettings);
    }

    private void sendRefreshMapMsg(final OsmandMapMiniLayer.DrawSettings drawSettings, int delay) {
...
//        }
    ...
    public void setTrackBallDelegate(net.osmand.plus.views.OsmandMapTileView.OnTrackBallListener trackBallDelegate) {
        this.trackBallDelegate = trackBallDelegate;
    }

    public void setOnLongClickListener(net.osmand.plus.views.OsmandMapTileView.OnLongClickListener l) {
        this.onLongClickListener = l;
    }

    public void setOnClickListener(net.osmand.plus.views.OsmandMapTileView.OnClickListener l) {

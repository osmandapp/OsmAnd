...
public class TransportFerryHelper {
	...
	public static double getTravelSpeed(TransportRoute route, double defaultSpeed) {
		int duration = isFerry(route) ? FerryRoutingHelper.parseDuration(route.getTags().get(FerryRoutingHelper.DURATION_TAG),
				route.getDistance()) : 0;
	...
	public static double getCrossingWaitTime(TransportRoutingConfiguration cfg, TransportRoute route, int stop) {
		...
		return crossing == null ? 0 : cfg.getWaitTime(FerryRoutingHelper.FERRY, Integer.parseInt(crossing[0]));
	}

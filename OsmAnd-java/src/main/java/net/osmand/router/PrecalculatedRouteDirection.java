...
//			indexedPoints.registerObjectXY(parent.pointsX.get(i), parent.pointsY.get(i), pointsX.size() - 1);
	...
	public float getDeviationDistance(int x31, int y31, int ind) {
		...
		if(ind < pointsX.length - 1 && ind != 0) {
			...
			PointInt31 proj = MapUtils.getProjectionPoint31(x31, y31, pointsX[ind], pointsY[ind], pointsX[nind], pointsX[nind]);
			distToPoint = (float) BinaryRoutePlanner.squareRootDist(x31, y31, proj.x, proj.y) ;
		}
		return distToPoint;
	}

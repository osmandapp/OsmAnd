	...
	public GpxRouteApproximation gpxApproximation() throws IOException {
		...
		while (currentPoint != null && currentPoint.pnt != null) {
			...
			for (MinDistResult currentSegment : lookAheadSegments) {
				long segmentId = calcSegmentId(currentSegment.segment);
				currentSegment.minDist /= (double) segmentPopularity.get(segmentId);
			...
			if (bestSegment.minDist > minPointApproximation) {
				...
				currentPoint = findNextRoutablePoint(nextIndex);
				continue;
			}
...
//		return Math.abs(gpxAngle - roadAngle) / Math.PI;
	...
	private GpxPoint findNextRoutablePoint(int searchStart) throws IOException {
		for (int i = searchStart; i < gpxPoints.size(); i++) {
			if (initRoutingPoint(gpxPoints.get(i), minPointApproximation)) {
	...
	private long calcSegmentId(RouteSegmentResult segment) {
		return ((segment.getObject().getId() / 64) << 32) + (segment.isForwardDirection() ? 1 : 0)
				+ (segment.getStartPointIndex() << 16) + (segment.getEndPointIndex() << 8);
	}
}
